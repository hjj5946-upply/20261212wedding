 ...
 * 사진 위에 덮는 투명 레이어.
 *
 * 카카오 인앱 브라우저(Android WebView)는 길게 누를 때 웹페이지가 아니라 앱이 직접
 ...
 *
 * 사용 규칙
 * - 부모에 `relative`(또는 absolute/fixed)가 있어야 한다. 없으면 더 바깥의
 *   positioned 조상까지 올라가서 엉뚱한 영역을 덮는다.
 * - 사진 위에 얹을 UI(버튼 등)는 이 컴포넌트보다 뒤에 렌더하거나 더 높은 z-index를 준다.
 * - 레이아웃·스크롤에는 영향이 없다. 클릭은 부모로 그대로 버블링된다.
 */
